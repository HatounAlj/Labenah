...
Turtle.display = function() {
      /*
      The parameters of drawImage method:
...
*/
   ...
   var img = new Image();
       img.src = 'magic-tool-extracted-image-without-background.png';
          Turtle.ctxDisplay.beginPath();
Turtle.ctxDisplay.drawImage(img,Turtle.x,Turtle.y,100,100);
...
Turtle.ctxDisplay.drawImage(img,Turtle.x,Turtle.y,100,100);
 ...
 //Updating the frame 
 updateFrame();
      ctx.clearRect(Turtle.x,Turtle.y,width,height);
 //Drawing the image 
 ctx.drawImage(character,srcX,srcY,width,height,Turtle.x,Turtle.y,width,height);     
}
...
Turtle.step = function(command, values) {
  ...
  switch (command) {
    case 'FD':  // Forward
      if (Turtle.penDownValue) { // go back to the first pos
